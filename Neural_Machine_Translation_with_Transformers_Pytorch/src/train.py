...
def train_model(config, model, train_loader, val_loader, optim, loss_fn, device, summary_writer, src_tokenizer, tgt_tokenizer):
    ...
    if config["preload"]:
        model_filename = f'{config["model_dir"]}_{config["preload"]}.pt'  # preload from this epoch
    ...
    for epoch in range(initial_epoch, config["epochs"]):
        ...
        for batch in batch_iterator:
            ...
            global_step += 1
        # run validation loop
        val_loss = run_validation(model, val_loader, loss_fn, device, summary_writer, src_tokenizer, tgt_tokenizer)
